...
- (NSString *)installedVersion
{
    ...
    [venueList enumerateKeysAndObjectsUsingBlock:^void(id key, id value, BOOL *stop) {
        NSDictionary *venueInfo = (NSDictionary *)value;
        NSString *assetVersion = [venueInfo objectForKey:@"assetVersion"];
        if (!latestVersion) {
            latestVersion = assetVersion;
        }
        
        if ([latestVersion compare:assetVersion] == NSOrderedAscending) {
            latestVersion = assetVersion;
        }
    }];
